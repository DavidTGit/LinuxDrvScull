...
static struct work_data_delayed {
...
{
	struct work_data * tmp= container_of(ptr,struct work_data,worker);
    
	if (! jiq_print (tmp->data))
		return;
...
static void jiq_print_wq_delayed(struct work_struct *ptr)
{
	struct work_data_delayed * tmp= 
		container_of(ptr,struct work_data_delayed,worker.work);
    
	if (! jiq_print (tmp->data))
		return;
    
	schedule_delayed_work(&tmp->worker, tmp->data->delay);
}
                   ...
                   int len, int *eof, void *data)
{
	...
	jiq_data.delay = 0;
	jiq_work.data=&jiq_data;
    
	prepare_to_wait(&jiq_wait, &wait, TASK_INTERRUPTIBLE);
	schedule_work(&jiq_work.worker);
	schedule();
                   ...
                   int len, int *eof, void *data)
{
	...
	jiq_work_delayed.data=&jiq_data;
    
	prepare_to_wait(&jiq_wait, &wait, TASK_INTERRUPTIBLE);
	schedule_delayed_work(&jiq_work_delayed.worker, delay);
	schedule();
	finish_wait(&jiq_wait, &wait);
...
static int jiq_init(void)
{
	...
	INIT_WORK(&jiq_work.worker, jiq_print_wq);
	INIT_DELAYED_WORK(&jiq_work_delayed.worker, jiq_print_wq_delayed);
