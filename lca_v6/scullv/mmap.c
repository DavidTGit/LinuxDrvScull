...
#define QSET_MASK(order) ( ~(0xfffffffful << (order)))
#define QSET_PAGE(page_nr,order) ((page_nr) & QSET_MASK(order)) 
#define QSET_OFFSET(page_nr,order) ((page_nr) >> (order))
/*
...
int scullv_vma_fault(struct vm_area_struct *vma,
                                struct vm_fault *vmf)
{
	unsigned long offset,qset_pages;
	struct scullv_dev *ptr, *dev = vma->vm_private_data;
	

	struct page *page;
	void *pageptr = NULL; /* default to "missing" */
	int rev = VM_FAULT_SIGBUS;

	down(&dev->sem);
	offset = ((unsigned long)vmf->virtual_address - vma->vm_start)+ (vmf->pgoff << PAGE_SHIFT) ;
	 ...
	 * accessing the hole.
	 */
	qset_pages=dev->qset<<dev->order;
	offset >>= PAGE_SHIFT; /* offset is a number of pages */
	for (ptr = dev; ptr && offset >= qset_pages;) {
		ptr = ptr->next;
		offset -= qset_pages;
	}
	if (ptr && ptr->data) pageptr = ptr->data[QSET_OFFSET(offset,dev->order)];
	if (!pageptr) goto out; /* hole or end-of-file */

    page=vmalloc_to_page(pageptr+(QSET_PAGE(offset,dev->order)<<PAGE_SHIFT));
